...
/// vendidas pelo DingFood) e o header x-polling-merchants (app centralizado, 1 client_id pra
    ...
    IUnitOfWork unitOfWork) : IIfoodOrderClient
{
    ...
    // DingFood só vende comida — categorias do módulo Grocery (varejo) ficam de fora.
    ...
    private async Task ExecuteWithLogAsync(string className, string methodName, object? payload, Func<Task> action)
    {
        ...
        catch (Exception ex)
        {
            try
            {
                var logEntry = new DingFood.Domain.Entities.LogTracker(0)
                {
                    ClassName = className,
