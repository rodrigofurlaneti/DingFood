            ...
            async (userIdBox) =>
            {
                ...
                if (branch is not null)
                {
                    ...
                    if (mapping?.IsActive == true && !string.IsNullOrWhiteSpace(mapping.MerchantId))
                    {
                        ...
                        var replacements = new List<DingFood.Domain.Entities.IfoodOpeningHours>();
                        foreach (var shift in remote.Shifts)
                        {
                            var created = DingFood.Domain.Entities.IfoodOpeningHours.Create(request.BranchId, shift.DayOfWeek, shift.Start, shift.DurationMinutes);
