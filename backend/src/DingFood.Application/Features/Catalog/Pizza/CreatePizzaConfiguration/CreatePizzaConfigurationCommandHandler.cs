...
namespace DingFood.Application.Features.Catalog.Pizza.CreatePizzaConfiguration;
    ...
    public override Task<Result<long>> Handle(CreatePizzaConfigurationCommand request, CancellationToken cancellationToken) =>
        ExecuteWithLogAsync(
            ...
            async (userIdBox) =>
            {
                ...
                var configuration = DingFood.Domain.Entities.PizzaConfiguration.Create(request.ProductId);
