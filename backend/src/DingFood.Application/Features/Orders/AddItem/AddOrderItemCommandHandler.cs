...
namespace DingFood.Application.Features.Orders.AddItem;
...
internal sealed class AddOrderItemCommandHandler : BaseCommandHandler<AddOrderItemCommand>
{
    ...
    private async Task<Result> HandleCoreAsync(AddOrderItemCommand request, CancellationToken cancellationToken)
    {
        ...
        var customizations = DingFood.Application.Features.Catalog.ProductExtras.ProductCustomizationResolver.Resolve(product, request.OptionalExtraIds, request.BoostIds);
