...
using DingFood.Application.Features.Cash.CloseSession;
using DingFood.Application.Features.Cash.GetHistory;
using DingFood.Application.Features.Cash.GetOpenSession;
using DingFood.Application.Features.Cash.ReviewSession;
using DingFood.Application.Features.Cash.GetSummary;
using DingFood.Application.Features.Cash.OpenSession;
using DingFood.Application.Features.Cash.RegisterMovement;
    ...
    IUnitOfWork unitOfWork) : ApiController(mediator)
{
    ...
    {
        var result = await Mediator.Send(new DingFood.Application.Features.Cash.GetRegisters.GetCashRegistersQuery(branchId), ct);
        return result.IsFailure ? HandleFailure(result) : Ok(result.Value);
    }

    [Authorize(Roles = ManagerRoles)]
    [HttpPost("registers")]
    public async Task<IActionResult> CreateRegister(DingFood.Application.Features.Cash.GetRegisters.CreateCashRegisterCommand command, CancellationToken ct)
    {
