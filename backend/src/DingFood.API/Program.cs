...
using DingFood.API.Services;
using DingFood.Application;
using DingFood.Application.Abstractions.Security;
using DingFood.Domain.Entities;
using DingFood.Domain.Repositories;
using DingFood.Infrastructure;
using System.Text;
...
static void ConfigureAuthentication(WebApplicationBuilder builder)
{
    ...
    builder.Services.AddAuthorization(options =>
    {
        ...
        foreach (var code in DingFood.Domain.Constants.FeatureCodes.All.Concat(DingFood.API.Authorization.FeatureAccessConvention.Controllers.Values).Distinct())
            options.AddPolicy($"Feature:{code}", policy =>
            {
                policy.RequireAuthenticatedUser();
                policy.Requirements.Add(new DingFood.API.Authorization.FeatureRequirement(code));
            });
    });
    builder.Services.AddScoped<Microsoft.AspNetCore.Authorization.IAuthorizationHandler,
        DingFood.API.Authorization.FeatureAuthorizationHandler>();
}
...
static void ConfigureSwagger(WebApplicationBuilder builder)
{
    ...
    {
        options.SwaggerDoc("v1", new OpenApiInfo { Title = "DingFood API", Version = "v1" });
