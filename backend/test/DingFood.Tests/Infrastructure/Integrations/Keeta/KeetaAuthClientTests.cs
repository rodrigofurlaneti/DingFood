...
using DingFood.Tests.Infrastructure.Integrations.TestSupport;
using Xunit;

namespace DingFood.Tests.Infrastructure.Integrations.Keeta;
...
public sealed class KeetaAuthClientTests
{
    ...
    public async Task GetAuthorizationUrlAsync_Success_ShouldBuildQueryWithEscapedParamsAndReturnUrl()
    {
        ...
        var url = await _client.GetAuthorizationUrlAsync(1, 2, "https://DingFood.example/callback?companyId=1&branchId=2", CancellationToken.None);

        url.Should().Be("https://keeta.example/authorize?token=abc");
        LastRequest.RequestUri!.ToString().Should().Contain("clientId=client-1");
        LastRequest.RequestUri!.ToString().Should().Contain(Uri.EscapeDataString("https://DingFood.example/callback?companyId=1&branchId=2"));
