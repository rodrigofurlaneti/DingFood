...
namespace DingFood.Tests.Infrastructure.Integrations.Asaas;
...
public sealed class AsaasAuthClientTests
{
    ...
    [Fact]
    public void Constructor_UserAgentNotYetSet_ShouldAddDingFoodUserAgent()
    {
        _env.EnvironmentName.Returns(Environments.Development);

        var client = CreateClient(ValidSettings());

        client.Client.DefaultRequestHeaders.UserAgent.Should().ContainSingle(u => u.Product!.Name == "DingFood");
    }
