...
public sealed class IfoodOrderClientTests
{
    ...
    public async Task PollEventsAsync_NoContent_ShouldReturnEmptyWithoutError()
    {
        ...
        events.Should().BeEmpty();
        await _logRepository.DidNotReceive().AddAsync(Arg.Any<DingFood.Domain.Entities.LogTracker>(), Arg.Any<CancellationToken>());
    }
    ...
    public async Task PollEventsAsync_HttpFailure_ShouldSwallowLogAndReturnEmpty()
    {
        ...
        events.Should().BeEmpty();
        await _logRepository.Received(1).AddAsync(Arg.Is<DingFood.Domain.Entities.LogTracker>(l => !l.IsSuccess), Arg.Any<CancellationToken>());
