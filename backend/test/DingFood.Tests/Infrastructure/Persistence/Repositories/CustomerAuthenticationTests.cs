...
using DingFood.Application.Features.Auth.CustomerRefresh;
...
public sealed class CustomerAuthenticationTests : RepositoryTestBase
{
    ...
    public async Task Refresh_RotatesCustomerToken_WithoutAdministrativeIdentity_AndRejectsReplay()
    {
        ...
        var adminRefresh = new DingFood.Application.Features.Auth.Refresh.RefreshTokenCommandHandler(
        ...
        var crossSession = await adminRefresh.Handle(
            new DingFood.Application.Features.Auth.Refresh.RefreshTokenCommand(result.Value.RefreshToken), default);
