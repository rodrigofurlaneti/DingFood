...
using DingFood.Application.Features.Integrations.Keeta.Authorization.RequestAuthorizationUrl;
using DingFood.Domain.Repositories;
using Xunit;

namespace DingFood.Tests.Application.Features.Integrations.Keeta.Authorization.RequestAuthorizationUrl;
...
public sealed class RequestKeetaAuthorizationUrlCommandHandlerTests
{
    ...
    {
        var command = new RequestKeetaAuthorizationUrlCommand(1, 2, "https://app.DingFood.com/api/keeta/oauth/callback");
        ...
        await _authClient.Received(1).GetAuthorizationUrlAsync(
            1, 2, "https://app.DingFood.com/api/keeta/oauth/callback?companyId=1&branchId=2", Arg.Any<CancellationToken>());
    }

    [Fact]
    public async Task Handle_RedirectUriWithExistingQueryString_ShouldAppendCorrelationWithAmpersand()
    {
        var command = new RequestKeetaAuthorizationUrlCommand(1, 2, "https://app.DingFood.com/callback?foo=bar");
        _authClient.GetAuthorizationUrlAsync(1, 2, Arg.Any<string>(), Arg.Any<CancellationToken>())
        ...
        result.IsSuccess.Should().BeTrue();
        await _authClient.Received(1).GetAuthorizationUrlAsync(
            1, 2, "https://app.DingFood.com/callback?foo=bar&companyId=1&branchId=2", Arg.Any<CancellationToken>());
    }
}
