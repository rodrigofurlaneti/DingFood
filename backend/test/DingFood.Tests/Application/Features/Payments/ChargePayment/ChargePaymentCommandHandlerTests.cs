...
namespace DingFood.Tests.Application.Features.Payments.ChargePayment;
...
public sealed class ChargePaymentCommandHandlerTests
{
    ...
    public async Task Handle_GatewayThrowsException_ShouldPropagateExceptionButStillLogViaFinally()
    {
        ...
        await _logRepository.Received(1).AddAsync(Arg.Any<DingFood.Domain.Entities.LogTracker>());
        await _unitOfWork.Received(1).CommitAsync(Arg.Any<CancellationToken>());
    }
}
