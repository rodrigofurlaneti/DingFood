...
using DingFood.Application.Features.Billing.RegisterPartialPayment;
...
public sealed class RegisterPartialPaymentCommandSteps
{
        ...
        decimal amount, long orderId, long cashSessionId, long paymentMethodId, long employeeId)
    {
        ...
        var availability = new Moq.Mock<DingFood.Application.Features.Cash.IPaymentMethodAvailability>();
        availability.Setup(x => x.ValidateAsync(It.IsAny<long>(), It.IsAny<IReadOnlyCollection<long>>(), It.IsAny<CancellationToken>())).ReturnsAsync(DingFood.Domain.Primitives.Result.Success());
