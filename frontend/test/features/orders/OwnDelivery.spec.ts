...
test('administrator configures rates, registers a driver and assigns an order', async ({ page }) => {
  ...
  await page.getByRole('tab', { name: 'Modelo de pagamento' }).click();
  ...
  await page.screenshot({ path: 'test-results/own-delivery-desktop.png', fullPage: true, animations: 'disabled' });
  await page.getByRole('tab', { name: 'Diárias a pagar' }).click();
  await expect(page.getByRole('button', { name: 'Registrar diária' })).toBeVisible();
  await page.getByRole('tab', { name: 'Modelo de pagamento' }).click();
  await expect(page.getByLabel('R$/KM', { exact: true })).toHaveValue('8.5');
  await page.setViewportSize({ width: 390, height: 844 });
  await expect(page.getByRole('button', { name: 'Salvar configuração' })).toBeVisible();
  expect(await page.evaluate(() => document.documentElement.scrollWidth <= window.innerWidth)).toBe(true);
  await page.screenshot({ path: 'test-results/own-delivery-mobile.png', fullPage: true, animations: 'disabled' });
});
