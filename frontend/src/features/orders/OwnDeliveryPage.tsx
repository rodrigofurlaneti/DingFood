...
function DeliveryWorkspace() {
    ...
    const mutation = useMutation({
        ...
        onSuccess: async () => { setNotice("Salvo com sucesso."); await queryClient.invalidateQueries({ queryKey: key }); },
        onError: (e: Error) => setNotice(e.message),
    });
    const save = (path: string, method: string, body: unknown) => { setNotice(""); mutation.mutate({ path, method, body }); };
    ...
    return <main className="own-delivery">
        ...
        <header><h1>Motoboys e taxas de entrega</h1><p>Configurações exclusivas da filial e marca selecionadas.</p><Link to="/delivery">Voltar ao painel de entregas</Link></header>
        {error && <p role="alert">{error.message}</p>}
        {notice && <p role="status">{notice}</p>}
        {(drivers.isLoading || config.isLoading) && <p>Carregando logística…</p>}
        <section><h2>{editing ? "Editar motoboy" : "Cadastrar motoboy"}</h2>
        ...
        <section><h2>Modelo de pagamento</h2><p>A taxa por KM é cobrada no pedido e registrada como custo por entrega. A diária é paga uma vez por motoboy/data e não adiciona taxa ao cliente.</p>
